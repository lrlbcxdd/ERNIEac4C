...
class Ernierna(nn.Module):
    def __init__(self, hidden_size=128, device='cuda:2'):
        ...
        self.filter_sizes = [2, 4, 8, 16, 32, 64, 128, 256]  # 0.9162 tmux1:512-256  tmux2:512-128
        # self.filter_sizes = [1, 2, 4, 8, 16, 32, 64, 128]  # 0.9081
        self.num_filters = 64
        self.convs_attn = nn.ModuleList(
            [nn.Conv2d(1, self.num_filters, (k, self.length - 2)) for k in self.filter_sizes])

        self.fc = nn.Sequential(
            nn.Linear(len(self.filter_sizes) * self.num_filters , len(self.filter_sizes) * self.num_filters // 4),
            nn.BatchNorm1d(len(self.filter_sizes) * self.num_filters // 4),
            nn.ReLU(),
            nn.Linear(len(self.filter_sizes) * self.num_filters // 4, 2)
        )

        self.linear_in = nn.Sequential(
            nn.Linear(self.emb_dim , (self.length-2) * 2),
            nn.GELU(),
            nn.LayerNorm((self.length-2) * 2)
        )

    def to_matrix(self,z):
        ab = self.linear_in(z)  # torch.Size([4, l*2])
        a, b = ab.chunk(2,dim = -1) # torch.Size([4, l])
        bsz , n, d =a.shape
    ...
    def forward(self, one_d, two_d):  # torch.Size([16, 1, length + 2])
        ...
        all_matrix = self.to_matrix(embedding)
        rna_attn_map_embedding = rna_attn_map_embedding[:, 1:416, 1:416]
        rna_attn_map_embedding = rna_attn_map_embedding.unsqueeze(dim=1)
        all_matrix = all_matrix.unsqueeze(dim=1)

        rna_attn_map_embedding = rna_attn_map_embedding + all_matrix
    ...
    for epoch in range(epoch_num):
        ...
        if test_acc > best_test_acc:
            ...
            filename = '{},{}, {}[{:.4f}].pt'.format('cls_Atten_model_{}_SEED={}_data{}'.format(half_length * 2 + 1 ,SEED,dataset) + ', epoch[{}]'.format(epoch + 1),model_name, 'ACC',
                                                  test_performance[0])
            save_path_pt = os.path.join(f'/mnt/sdb/home/lrl/code/Saved_models/Models/cls-matrix/', filename)
            ...
            save_path_roc = os.path.join(f'/mnt/sdb/home/lrl/code/Saved_models/ROC/cls-matrix/', filename)
            save_path_prc = os.path.join(f'/mnt/sdb/home/lrl/code/Saved_models/PRC/cls-matrix/', filename)
