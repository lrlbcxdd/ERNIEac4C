...
class Ernierna(nn.Module):
    def __init__(self, device='cuda:2'):
        ...
        # self.filter_sizes = [2, 4, 8, 16, 32, 64, 128, 256] # 0.9216
        # self.filter_sizes = [1, 2, 4, 8, 16, 32, 64, 128] # filter = 96 bs4:0.9243 bs8:0.9189 bs2:0.9081
        # self.filter_sizes = [8, 16, 32, 64, 128, 192, 256, 384] # 0.9243
        self.filter_sizes = [1, 2, 4, 8, 16, 32, 64, 128] # filter = 64 bs4:0.9243 bs8:0.9189 bs2:0.9081
        self.num_filters = 64
        ...
        self.fc = nn.Sequential(
            nn.Linear(len(self.filter_sizes) * self.num_filters + self.emb_dim, len(self.filter_sizes) * self.num_filters),
    ...
    for epoch in range(epoch_num):
        ...
        if test_acc > best_test_acc:
            ...
            filename = '{},{}, {}[{:.4f}].pt'.format('cls_Atten_model_{}_SEED={}_data{}'.format(half_length * 2 + 1 ,SEED,dataset) + ', epoch[{}]'.format(epoch + 1),model_name, 'ACC',
                                                  test_performance[0])
            save_path_pt = os.path.join(f'/mnt/sdb/home/lrl/code/Saved_models/Models/415onlyAttn/', filename)
            ...
            save_path_roc = os.path.join(f'/mnt/sdb/home/lrl/code/Saved_models/ROC/415onlyAttn/', filename)
            save_path_prc = os.path.join(f'/mnt/sdb/home/lrl/code/Saved_models/PRC/415onlyAttn/', filename)
