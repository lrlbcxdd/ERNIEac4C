    ...
    for epoch in range(epoch_num):
        ...
        if test_acc > best_test_acc:
            ...
            filename = '{},{}, {}[{:.4f}].pt'.format('Atten_model_415_SEED={}'.format(SEED) + ', epoch[{}]'.format(epoch + 1),model_name, 'ACC',
                                                  test_performance[0])
            save_path_pt = os.path.join(f'ac4c_models/Models/cls-Atten/', filename)
            if parallel_train:
            ...
            save_path_roc = os.path.join(f'ac4c_models/ROC/cls-Atten', filename)
            save_path_prc = os.path.join(f'ac4c_models/PRC/cls-Atten/', filename)
