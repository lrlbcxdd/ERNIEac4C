...
class Ernierna(nn.Module):
    ...
    def forward(self, one_d, two_d):  # torch.Size([16, 1, length + 2])
        ...
        # 分类器
        output = self.fc[:1](torch.cat((embedding, rna_attn_map_embedding), dim=1))
...
if __name__ == '__main__':
    ...
    train_iter, val_iter, test_iter, max_len = ac4c_loader.load_ac4c_data(halfLength=207, batchsize=32)

    print('extracting...')

    # load model
    model = Another_Transformer.Transformer(max_len=max_len).to('cuda:3')
    print('Model Loading Done!!!')

    model_path = "/mnt/sdb/home/lrl/code/Saved_models/Models/Transformer/model_415, epoch[26],Transformer, ACC[0.8216].pt"
    ...
    torch.save(all_embedding, '../pt/Transformer_embedding.pt')
    torch.save(all_labels, '../pt/Transformer_labels.pt')
