...
class Ernierna(nn.Module):
    ...
    def forward(self, one_d, two_d):  # torch.Size([16, 1, length + 2])
        ...
        # 分类器
        output = self.fc[:3](torch.cat((embedding, rna_attn_map_embedding), dim=1))
...
class LSTMModel(nn.Module):
    def __init__(self,max_len, vocab_size=5, emb_dim=100, hidden_dim=64):
        ...
        self.block = nn.Sequential(
            nn.Linear(64 * max_len, 1024),  # 415:26560 201:12864 101:6464 51:3264
            nn.BatchNorm1d(1024),
            nn.LeakyReLU(),
            nn.Linear(1024, 128),
            nn.BatchNorm1d(128),
...
if __name__ == '__main__':
    ...
    # load model
    model = LSTMModel(max_len=max_len).to('cuda:3')
    print('Model Loading Done!!!')

    model_path = "/mnt/sdb/home/lrl/code/Saved_models/Models/LSTM/model_415, epoch[11],LSTM, ACC[0.8270].pt"
    ...
    torch.save(all_embedding, '../pt/lstm_embedding.pt')
    torch.save(all_labels, '../pt/lstm_labels.pt')
