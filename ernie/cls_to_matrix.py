...
class Ernierna(nn.Module):
    def __init__(self, hidden_size=128, device='cuda:2'):
        ...
        self.filter_sizes = [2, 4, 8, 16, 32, 64, 128, 256]  # 0.9162 512-256 415：0.9054 512-128 415:0.8972 512-128 417:0.9162 512-256 417:0.8918
        ...
        self.linear_in = nn.Sequential(
            nn.Linear(self.emb_dim , (self.length) * 2),
            nn.GELU(),
            nn.LayerNorm((self.length) * 2)
        )

    def to_matrix(self,z):
        ab = self.linear_in(z)  # torch.Size([4, l*2])
        a, b = ab.chunk(2,dim = -1) # torch.Size([4, l])
        x = torch.einsum("...bc,...de->...bce", a, b)    # torch.Size([4, l, l])
        del a,b
        return x
    ...
    def forward(self, one_d, two_d):  # torch.Size([16, 1, length + 2])
        ...
        cls_matrix = self.to_matrix(embedding)
        rna_attn_map_embedding = rna_attn_map_embedding.unsqueeze(dim=1)
        cls_matrix = cls_matrix.unsqueeze(dim=1)

        rna_attn_map_embedding = rna_attn_map_embedding + cls_matrix
